...
class frogAtX(highwayCenterX:Number)  below(highwayBottom:Number)
    ...
    def frogImage:Graphic2D = drawableImageAt(startPoint)size(frogWidth@frogHeight)
...
def froggerGame: GraphicApplication = object {
    ...
    filledRectAt(highwayLeft @ highwayTop)
    ...
    def theFrog:Frog = frogAtX (highwayLength/2) below (highwayBottom)
    ...
    for(1..numLanes) do {lane:Number ->
        def laneSpeed:Number = randomNumberFrom(0.033)to(0.1)
