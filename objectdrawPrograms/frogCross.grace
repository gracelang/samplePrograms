...
class frog.atX(highwayCenterX:Number)   below(highwayBottom:Number)
    ...
    def frogImage:Graphic2D =
        drawableImageAt(startPoint)size(frogWidth@frogHeight)
...
class vehicle.startAt(startPt: Point) 
        image(carURL:String) 
        going (speed:Number)
    ...
    method start -> Done {
        def vehicleImage:Graphic2D = 
            drawableImageAt(startPt)size(carWidth@carHeight)
                   url(carURL)
                   on(canvas)
...
class laneFiller.centerY(y: Number) 
    ...
    method start -> Done {
        ...
        animator.while {count < maxVehicles} pauseVarying {randGapGenerator} do {
            def newVehicle: Animated = vehicle.startAt(startPoint) image(url) 
                going (speed)
...
def froggerGame: GraphicApplication = object {
    inherit graphicApplicationSize(800@600)
    ...
    filledRectAt(highwayLeft@highwayTop)size(highwayLength@highwayWidth)on(canvas)
    ...
    def theFrog:Frog = frog.atX (highwayLength/2) below (highwayBottom)
    ...
    for(1..numLanes) do {lane ->
        ...
        def nextLane: Animated =
            laneFiller.centerY(highwayTop + lane * laneWidth - carHeight)
                laneEnds(highwayLeft,highwayRight)
                carURL (url)
                carSize (carWidth, carHeight)
                going (laneSpeed)
